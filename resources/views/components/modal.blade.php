            ...
            >
                @if(isset($title))
                    <div class="aura-modal-header px-6 pt-5 pb-4 border-b border-aura-surface-200 flex items-center justify-between gap-4">
                        ...
                        @if($closeable)
                            <button type="button" class="aura-modal-close shrink-0 w-8 h-8 flex items-center justify-center border-none bg-transparent text-aura-surface-400 rounded-aura-sm cursor-pointer aura-transition-fast hover:bg-aura-surface-100 hover:text-aura-surface-900" x-on:click="open = false" aria-label="{{ __('aura-ui::messages.close') }}">
