...
@php
    ...
    $icons = [
        ...
        // Targets referenced by $heroiconMap that were never drawn: without these
        // the aliases below silently fell back to the "unknown icon" circle.
        ...
        // Identity aliases ('sparkles' => 'sparkles') that never had an icon behind
        // them: the lookup only searches $icons, so they rendered the circle too.
    ...
    // Heroicon outline → Lucide name mapping (fallback for Heroicon names)
    $heroiconMap = [
        'arrow-right-start-on-rectangle' => 'log-out',
        'arrow-left-start-on-rectangle' => 'log-in',
        'arrow-right-end-on-rectangle' => 'log-in',
    ...
    // Try Lucide name first, then Heroicon mapping, then fallback
    $resolvedName = $icons[$name] ?? null;
    if ($resolvedName === null && isset($heroiconMap[$name])) {
        $resolvedName = $icons[$heroiconMap[$name]] ?? null;
    }
