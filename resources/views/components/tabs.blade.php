    ...
    <div class="aura-tabs-list flex {{ $variant === 'vertical' ? 'flex-col border-r border-aura-surface-200 pr-4' : 'border-b border-aura-surface-200' }} gap-0" role="tablist">
        @foreach($tabs as $tab)
            <button
                type="button"
                role="tab"
                class="aura-tabs-trigger inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-aura-surface-600 border-b-2 border-transparent cursor-pointer bg-transparent aura-transition-fast hover:text-aura-surface-900 hover:border-aura-surface-300 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                :class="{ 'aura-tabs-active !text-aura-primary-600 dark:!text-aura-primary-400 !border-aura-primary-500 !font-semibold': activeTab === {{ Js::from($tab['name']) }} }"
